...
### 6b. 观战与中途加入（W24，用户拍板 2026-08-10）

> 拍板原话：「运行中房间可以观战，也可以中途加入（根据房间当前队伍情况，
> 比如红队满了，那就只能加入蓝队，可以自己选择职业）」。
> 在此之前 `JoinRoom` 对 `started` 房间只回一句「比赛已开始」——
> 单机房永远只有开局那批人。

```
对一个 started 房间发 JoinRoom
  → **进观战席**（名单上就是观战席成员，会话进 SessionPhase.Spectate）
  → 收到 MatchStart{ spectating: true, you: 当前跟随对象 }
  → 从下一份快照起收**观战段**

在观战席上发 JoinOngoing{ team, classId }
  → 那一队还坐得下  → 新建实体入场，**职业当场生效**
  → 队伍满了但有补位人机 → **顶替**那个席位（沿用它的身体与 playerId），
                            选的职业等**下一次复活/回合重置**才换
  → 都不成 → Rejected，且理由点名另一队还有几个位置
```

四条口径，每条都有对应的实现处与测试：

1. **观战段 = 两队可见集的交集**，不是并集（`isVisibleToSpectator`）。
   一行判据：**不是「尚未被任何敌人发现的潜行者」就进**。
   字面的并集会把潜行者**自己那一队**的可见性算进来（红队永远看得见自己的盗贼），
   于是任何人开第二个窗口坐进观战席就能给全场潜行者点名 —— 那是换了个入口的
   §4.1 透视。副产品：**观战段 ⊆ 任意一队的可见集**，`assertNoHiddenEntities`
   对它照抛不误（判据跟着受众走，见 `SnapshotAudience`）。
2. **观战席拿到什么**：实体段（交集）+ 事件流（同一套 `redactFor`）+ `EntityMeta`；
   **装备一律敌人视图**（他不是任何人的队友，§4.2 对他没有豁免）；
   **`self` 段一个字节都不发** —— 里面是冷却/GCD/焦点，而 §4.3「敌方技能冷却不发」
   是一条裁剪规则，用产品功能把它挖穿不划算；观战者没有身体、按不出技能，
   本来也不需要冷却条。`you` = 当前跟随对象，一个可跟的都没有时是 **0**
   （`NO_ENTITY` 哨兵，实体 id 从 1 起）。
3. **中途加入的职业为什么延后**：顶替人机时当场换职业 = 满血 + 满资源 +
   冷却清空 + 光环全清，而这条路径可以被反复触发（下一个观战者再顶一次）——
   一个免费复活的经济漏洞。兑现点是**「死 → 活」的状态跳变**（复活波次或回合重置，
   两条出口一视同仁）。⚠️ **竞技场默认单回合制**，服务器从不调 `resetRound` ——
   那一刻在一局竞技场里**不会到来**，顶替者整局用被顶替人机的职业；
   客户端文案必须照实说。
4. **只顶替 `fill`（补位）人机，绝不顶替 `disconnect`（掉线接管）的席位** ——
   后者是某个真人的角色，他攥着重连令牌随时会回来。大 BOSS 的人机席位靠
   「不在 `room.players` 名单里」被同一道筛排除。

观战席的生命周期与选手**不同**：断线不登记人机接管（没有席位可开）、
不广播 `PeerDisconnected`（他不是任何人的队友），但**照样登记重连宽限**
（同一个令牌回来仍是观战席）；宽限到期从名单里删掉而不是弃权判死
（他没有可规避的死亡统计，11.5 那条管的是有角色的人）。

> ⚠️ **已知的信息不对称，登记为 S10**：观战席是全知席位，而本仓库没有账号系统，
> 任何人都能开第二个窗口坐进去。缓解手段（转播延迟 / 只给单队视角 / 房主开关）
> 都要重新裁决，本批不做。
...
### 8.7 观战与中途加入（W24，2026-08-11）

语义见 §6b，这里只记协议面。**四处改动全是加法**，没有任何字段变形或删除。

```ts
// 客户端 → 服务器（SPECTATE_ONLY —— 只有观战席能发）
| { t: 'JoinOngoing'; team: 'red' | 'blue'; classId: ClassId }

// 服务器 → 客户端（三处可选字段）
| { t: 'MatchStart'; …; spectating?: boolean }
  RoomPlayerView += bot?: boolean
  RoomList 行     += joinableSeats: number
```

- **`JoinOngoing` 刻意没有 `seat: 'spectator'` 这个取值**：进观战席就是对一个
  `started` 房间发 `JoinRoom`，留一个什么都不做的取值等于留一条死分支。
  反方向（战斗席退回观战席）同样不给 —— 中途退出走 `LeaveMatch` 按 11.5 淘汰，
  不能靠改席位规避死亡统计。大乱斗没有两队，`team` 的两个取值都读作「参战」。
  `classId` 只在 codec 里验「非空字符串」，能不能选由 sim 的 `isPlayableClass` 判
  （与 `SelectClass` 同一条分工）。
- **`MatchStart.spectating` 客户端必须读**：`you` 对观战者是「当前在看谁」而不是
  「我操控的角色」—— 不区分的话，观战者的客户端会对着别人的角色做本地预测并把
  `Input` 发上来（服务器按阶段拒收，但画面会与快照打架）。
- **`RoomPlayerView.bot` 的判据是「有没有一条人机会话」**，不是名字前缀 ——
  名字是玩家可控的字符串，靠它认人机迟早被一个叫「人机7」的真人骗过。
  它只为**显示**（把按钮画成可点也顶替不了掉线接管的席位）。
- **`RoomList.joinableSeats` 与 `capacity - players` 不等价**：开局后队伍被人机
  补满，差值恒为 0，而真正坐得下的正是那些人机席位。没有这个字段，大厅只能
  画出「已开始 · 满员」—— 中途加入在 UI 上就不存在。
  `started && joinableSeats === 0` 的房间**仍然可以观战**（观战不占战斗席）。

**阶段机**：`Session` 加第四个阶段 `Spectate`，白名单从两张变三张 ——
`MATCH_ONLY`（要身体的全在这儿）/ `MATCH_OR_SPECTATE`（只有 `SpectateFollow`）/
`SPECTATE_ONLY`（只有 `JoinOngoing`）。逐条过完 `MATCH_ONLY` 的结论：
观战者一条都发不了，**`SetTarget` 尤其** —— 它是验收 #5 的探测通道
（见 `RoomServer.onSetTarget` 的 ★★），放行等于白送一个不需要身体的可见性探针。

**顺带修的一处静默**：`EntityMeta` 的补发判据从「只盯 `maxHealth`」（W26）扩成
**整块静态块的指纹**。顶替人机后要改名、换职业后 `classId`/`maxResources` 全变，
只比血量的话「换成同样血量的职业」一个字节都不重发 —— 客户端会顶着旧职业图标
和旧名字画一个新职业的人，而快照里的动态量是对的，所以没有任何一层会报错。
