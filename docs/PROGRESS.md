...
**423 个单元测试 + 71 项端到端验收全绿**（M1 14 + M2 14 + M3 12 + M4 7 + M5 11 + M6 13）。

**还没有的**：夺旗战场（M7）、特效与完整 HUD（M8）、统计与观战（M9），
...
**目标**：12.x 的完整夺旗流程。规则量比竞技场大，但结构更清晰。

### 建议的实现顺序

1. **`shared/src/sim/match/flag.ts`** —— 12.2 旗帜七状态机
   - 基地中 / 拔取中 / 携带中 / 掉落 / 归还中 / 交付中 / 重置中
   - ★ 己方旗帜不在基地时**不能交旗**（12.1 / 验收 #39）
   - ★ 旗手使用潜行、消失、完全无敌时**先掉旗再播放技能表现**（12.3 / 验收 #40）
2. **旗手限制** —— 不能潜行/隐身/坐骑/传送；位移技能距离降低但保留（12.3）
   - `SkillDef.forbiddenWhileCarryingFlag` 和 `dropsFlagOnUse` 在 M0 就已经标好了
3. **12.4 战场聚焦** —— 同时持旗超 60 秒后每 30 秒一层，最多 5 层（验收 #41）
4. **`shared/src/sim/match/respawn.ts`** —— 12.6 波次复活 + 3 秒保护
   - ★ 复活保护**不能**用于直接完成拔旗或交旗
5. **一张可缩放的大型夺旗地图** —— 6v6/8v8/12v12 共用（19.1）
...
- 本项目在中国大陆网络环境下开发，**git 推送需要走本地代理**。
  仓库的 `.git/config` 里已配置 `http.proxy = http://127.0.0.1:6789`（Clash 默认端口）。
...
2. 读 [01-development-plan.md](01-development-plan.md) 的 M7 一节和
   [06-modes-and-maps.md](06-modes-and-maps.md)（旗帜状态机、旗手限制、复活波次）
