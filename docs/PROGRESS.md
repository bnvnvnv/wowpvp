...
**598 个单元测试 + 106 项端到端验收全绿**（M1 14 + M2 14 + M3 12 + M4 7 + M5 11 + M6 13 + M7 23 + M8 12）。
...
## 👉 下一步：M9（统计、观战、异常处理、可访问性、素材许可）

**目标**：规格书 16.x / 17.x / 18.x。这是最后一个里程碑，也是唯一一个
**大部分工作不产生新玩法**的里程碑 —— 它把已有的东西变得可复盘、可旁观、可容错。

### 剩下的 4 条验收

| # | 内容 | 主要工作 |
|---|---|---|
| #5 | 未被发现的潜行目标不能被点击、Tab 或小地图选中 | 快照可见性裁剪（`net/visibility.ts`）。★ 小地图已经做好了准备：`Minimap.draw()` 只吃调用方给的列表，自己拿不到世界状态 |
| #50 | 战后统计含打断、驱散、控制、换装、装备争夺、夺旗贡献 | `shared/src/sim/stats.ts` + 16.1–16.4 |
| #51 | 外部素材均有许可记录 | 目前**没有任何外部素材**（docs/09 §5），所以这条要么天然满足、要么在引入素材时补 |
| #52 | 不存在等级、永久装备、付费属性或外观稀有度优势 | 设计审查。无账号系统即天然满足，但 17.1 要求**显式确认** |

### 建议的实现顺序

1. **`shared/src/sim/stats.ts`** —— 16.1–16.3 的计数器。
   ★ 16.4「**不能只按总伤害或击杀数评选最佳玩家**」是一条否定式规则，
   建议做法：最佳玩家的评分函数**不接收** damage/kills 之外的单一指标，
   而是按七个维度各自算分（最佳旗手/护送/防守/治疗/打断/装备争夺/综合）。
2. **`net/visibility.ts`** —— 验收 #5。快照裁剪是**服务器**职责，
   客户端拿不到的东西就不可能画出来。这也是唯一能真正满足 #5 的做法 ——
   客户端过滤等于把数据发过去再假装看不见。
3. **17.3 异常处理五类** —— 其中三类已经有实现：
   同时拾取（M6 `beginPickup`）、进入非法区域（M7 `dropFlag`）、
   施法完成瞬间目标失效（M2 `validateCast` 双重检查）。
   缺的是断线重连与「换装瞬间死亡」的状态唯一性。
4. **17.2 可访问性** —— 全键位重绑（`DEFAULT_BINDINGS` 已经是数据驱动的）、
   色盲模式、UI 缩放、减弱震动。★ 「危险区域不能只依赖颜色」M3/M8 已经做到了
   （虚线 + 叉号 + 字形三通道），这里要补的是**设置项**本身。
5. **观战** —— 只能跟随己方存活玩家。★ 这条与 #5 同源：
   观战镜头如果能自由飞，就等于给了透视。

### M9 的验收判据

`#5` / `#50` / `#51` / `#52`。其中 #52 是设计审查而非代码 ——
要在文档里写清「本项目没有账号系统、没有付费项、外观不影响属性」并逐条对照 17.1。
...
2. 读本文件的「👉 下一步：M9」一节，再看
   [01-development-plan.md](01-development-plan.md) 的 M9 表格
