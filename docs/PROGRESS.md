...
**490 个单元测试 + 94 项端到端验收全绿**（M1 14 + M2 14 + M3 12 + M4 7 + M5 11 + M6 13 + M7 23）。

**还没有的**：特效与完整 HUD（M8）、统计与观战（M9），
...
**目标**：15.x 与 14.x —— 让已经正确的规则变得**看得懂**。
到 M7 为止所有规则都对了，但玩家在屏幕上几乎看不出来。
...
2. 读 [01-development-plan.md](01-development-plan.md) 的 M8 一节和
   [07-client-render-camera.md](07-client-render-camera.md)（特效分层、画质档位、HUD）
