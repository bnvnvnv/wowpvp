...
**250 个单元测试 + 40 项浏览器端到端验收全绿**（M1 14 + M2 14 + M3 12）。

六类瞄准的输入流程也齐了：按 `5` 会进入落点预览，圆圈跟着鼠标走，
拖到墙后立刻变成**虚线 + 叉号 + 变暗 + 文字说明「缺少视线」**，左键点不下去。

还没有的：技能效果结算（伤害/治疗/光环/控制递减/驱散，M4），联网对战（服务器仍是 M0 的连通性桩）。
...
| M4 | 八职业技能真正跑起来、控制递减、驱散免疫 | ⬜ 未开始 ← **下一步** |
| M5 | 竞技场模式与地图 | ⬜ 未开始 |
...
## 👉 下一步：M4（效果系统、控制递减、驱散免疫）

**目标**：M0 写的 91 个技能真正产生效果。当前技能能释放、能被打断，
但**还不造成伤害** —— 效果层是空的。

### 建议的实现顺序

1. **`shared/src/sim/effects/index.ts`** —— 效果注册表
   - 每个 `EffectDef.kind` 一个处理器，`EffectDef` 是可辨识联合，**漏注册会编译报错**
   - 先注册 `damage` / `heal` / `applyAura` 三个，跑通一条伤害链
2. **`shared/src/sim/aura.ts`** —— 光环系统
   - 叠加、刷新、周期跳、受伤解除、护盾衰减与破裂
   - `StatusFlags` 改为由光环聚合产生，而不是像 M2 那样手动赋值
3. **`shared/src/sim/dr.ts`** —— 8.2 控制递减
   - 五类梯度、15 秒窗口。★ 定身与普通减速是**两条独立的链**
4. **`shared/src/sim/modifiers.ts`** —— 8.4 / 17.1 同类效果不无限叠加
   - 减速取最强、团队减伤取最强、伤害提高相乘（爆发窗口是设计意图）
5. **收敛 custom handler** —— 见下方技术债第 1 节，schema v1.1 之后 8 个可以数据化

### M4 的验收判据

覆盖验收 #23（控制递减、通用解控、驱散、免疫、治疗抑制正确结算）
与 #46（位移技能接入模拟循环后端到端验证）。

★ M4 完成后才应该做数值配平 —— 在此之前调数字没有意义（见技术债第 2 节）。
...
2. 读 [01-development-plan.md](01-development-plan.md) 的 M4 一节和
   [03-combat-system.md](03-combat-system.md) §4（控制递减、解控、驱散、叠加规则）
