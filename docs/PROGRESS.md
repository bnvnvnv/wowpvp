...
**384 个单元测试 + 58 项端到端验收全绿**（M1 14 + M2 14 + M3 12 + M4 7 + M5 11）。

**还没有的**：临时武装（M6）、夺旗战场（M7）、特效与完整 HUD（M8）、
统计与观战（M9），以及**联网对战** —— 服务器目前仍是 M0 的连通性桩，
所有对局都跑在客户端本地模拟里。
...
## 👉 下一步：M6（临时武装：军械箱、拾取、换装）

**目标**：10.x 的完整装备争夺。这是最容易被玩家找到漏洞的系统。

### 建议的实现顺序

1. **`shared/src/sim/loadout.ts`** —— 战场装备栏
   - 1 套不可删除的默认武器/护甲 + 2 武器 + 2 护甲 + 2 道具（10.6）
   - ★ 换装**不能**刷新普通攻击计时、取消后摇、重置冷却、恢复资源或清除负面
     （10.7 / 验收 #34）—— 这条要逐项写断言，是全项目最容易被利用的地方
2. **`shared/src/sim/arsenal.ts`** —— 军械箱与补给刷新
   - 固定可预测、双方距离对等、提前 5 秒预告（10.4）
   - ★ 职业锁定：不匹配时提示且**物品不消失**（10.2 / 验收 #29）
   - 拾取 2.2 米 / 0.8 秒，可被移动、控制、位移、死亡中断；**普通伤害不中断**（10.5）
3. **武装竞技场预设接入** —— `ArenaPreset.Armed` 时才生成补给点（验收 #28）
4. **10.10 死亡与回合重置** —— 死亡后临时装备随该玩家失效，不掉给敌人

### M6 的验收判据

覆盖验收 #28–#30、#33、#34、#36，并把 #37 补完。
...
2. 读 [01-development-plan.md](01-development-plan.md) 的 M6 一节和
   [05-equipment-system.md](05-equipment-system.md)（武器护甲映射表，由代码自动生成）
