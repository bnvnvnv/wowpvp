...
| X3 | `Damage.skillId`（死亡回顾显示技能名） | 拍板口径「来源不可见时 skillId 同 sourceId 一起抹，aura 同理」，一次定死 | ✅ |
| S7 | 光环 id 的隐身泄露口径 | 施加者不可见 → auraId 掩成 `HIDDEN_AURA_ID`；落 `snapshotEntity` + `AuraApplied` 裁剪 | ✅ |
| W18 | 复核他人姓名板施法条数据源 | 复核结论：`CastStarted` 事件流 → `beginCast` 注册表 → `castOf` 已被姓名板与目标框双双消费，**销账** | ✅ |
| A5/A4 | （评估项）转身速率上限 / 人机可见性过滤 | A5 动 sim 手感，需真机验证后定阈值；A4 给 `nearestFoe` 加视线/距离过滤 —— **两条仍留评估**，放批次五 | ⛔ 留 |

**判据达成**：`verify:m10` **16/16**（新增 #1e：来源不可见时 skillId + auraId 一并抹、掩成 "hidden" 的字节级断言）+ 每个新字段一条消费侧断言（m13 #23 W18、#24 X3、HitFeedback avoided 单测、visibility S7 掩码 ×3、codec 往返）。
