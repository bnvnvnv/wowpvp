...
| X21 | **排队窗过期无反馈**：0.4s 排队窗（P10）过期后按键仍是静默消失 —— 迟到 0.4 秒的失败提示可能比沉默更误导，**要不要提示、提示什么需拍板**，不是漏做 | `sim/casting.ts` 排队消费处注释 | 「按了没反应」残留的最后一角 | 🔧 **拍板了：要提示，但换一路说，不复用失败提示**（用户拍板批 2026-08-10，任务 D）。**sim + 协议 + 服务器三层已通，HUD 消费待下一波**。落地：`CastEvents` 新增 `onQueueExpired(caster, skill, { queued, waited })`，`tickCastQueue` 过期分支调它（`onFailed` 仍然不发 —— 迟到的 `onGlobalCooldown` 才是误导源，换通道不是加噪音）；新协议私信 `CastQueueExpired { skillId, waited }`（照 M10 全套：union + `ALL_SERVER_MESSAGE_KINDS` + `referencedEntities` 归私信一列返回 `[]`，零实体引用；刻意不给 `CastFailure` 加枚举值 —— 客户端 `FAIL_TEXT: Record<CastFailure,string>` 会因此编译不过，那是跨包破坏）；`MatchLoop` 的 cast sink 接上私发。bot 永不排队的三道保险一字未动（另加一条副作用测试钉住）。测试：`sim/castQueue.test.ts` +3、`MatchLoop.castQueue.test.ts` +2。**余账**：客户端消费（HUD 用什么形态说「刚才那一下没赶上」—— 弱提示/图标闪一下/什么都不做，`waited` 已备好给它判「差一点」），归下一波 HUD 批 |
