...
> 2026-09-05 更新：默认界面将团队竞技场限定为 1v1–5v5（单回合淘汰），大型团队
> 玩法为 6v6 / 8v8 / 12v12 夺旗。夺旗已启用 BOSS 战利品、随机补给和单局经验商店，
> 覆盖下文原规格中的「夺旗首版关闭临时装备」限制。当前实装细则见
> [可玩性审查](18-playability-audit.md)。
...
> ⚠️ **2026-08-11 修**：楼梯顶面原本写成 `PLAT_H − (i+1)*RISE` 倒着算，
> 浮点上差 1 个 ulp，实测**直走停在 y=0.90，第三级永远上不去** ——
> 台顶只有会跳的人到得了。现在改成逐级累加（`PILOT_STAIR_TOPS`）。
> 同一条公式当初被复制进了 P5 的三张主题图，详见 §10.6b 那张表的第一行。
> ★ 另：这张图的直线陷阱率是全仓最高的 **11.46%**（柱环正好落在 45° 上，
> 出生点直走撞的就是柱子的竖棱），机制与修法见总账 **X28**（docs/17 有全仓测量表），本批未动。

---

### 10.6 新增竞技场图池：四张主题图（P5，2026-08-10）

用户拍板：「地图美术帮我多做一些地图（各种不同风格的，各种地势和道具的）」。
§10.5 的试点手感成立，于是把地形从一张图铺开成**四张各自成立的主题图**。

> ⚠️ **本批交付的「风格」= 布局地势 + 装饰主题 + 昼夜 preset + 地面材质 四个轴。**
> 剩下的美术半边 —— 墙/柱的**贴图与材质分化**（Q 版高饱和方向）、装饰 instancing、
> 天空盒 —— 要素材侧动工，仍挂在 docs/17 的 P5 行上。

实现位置：`packages/shared/src/data/maps/themed.ts`（试炼环在 `arena.ts`，两者互不影响）。

| id | 名字 | preset / 地面 | 地势特征 | 装饰主题 | 适配档位 | 场地 | 出生点/队 |
|---|---|---|---|---|---|---|---|
| `arena_frost_outpost` | 雪原哨站 | `dawn` / `snow` | 开阔中场 + 两翼冰棱墙群（每翼 3 道，沿 z/x 交替）+ 中央缓坡高地 1.35m（= 胸口线，**不挡视线**，给站位不给掩体）| 哨站营地：帐篷 · 火塘 · 路牌 · 冰晶 · 针叶林 | 1v1 · 2v2 | 92 × 92 m | 2 |
| `arena_grove_altar` | 密林祭坛 | `day` / `grass` | 柱状树阵 28 棵（`pillar` 真碰撞体）+ 中央祭坛矮台 0.9m + 倒木**可跨路缘** 8 根（0.45m）| 林间祭坛：四角火盆 · 图腾 · 先祖废墟 · 鹿神龛（在林深处，见 §10.6b）· 蕨类 · 发光菌 | 3v3 · 4v4 · 5v5 | 134 × 134 m | 5 |
| `arena_lava_rift` | 熔岩裂谷 | `dusk` / `rock` | 中央大高台 26×26（2.25m，**整块挡视线**）+ **环形阶梯**（任何方向直走都上得去）+ 环谷视线墙 ×10，高低差主导 | 熔岩火盆：台顶火盆 · 岩浆池/河 · 黑曜石尖 · 骨堆 · 熔渣锅 | 6v6 · 7v7 · 8v8 · 9v9 | 176 × 176 m | 9 |
| `arena_ruins_colosseum` | 废墟角斗场 | `night` / `dirt` | 中央刻意空场 + 两翼各两座断塔（2.25m）+ 短桥 ×2（桥下净空 2.25m 可穿行）+ 半塌楼梯 ×4 + 断墙群 | 废料箱桶：木箱堆 · 酒桶 · 断柱 · 瓦砾 · 火把（night 下唯一暖光）| 8v8 · 9v9 · 10v10 · 11v11 · 12v12 | 190 × 190 m | 12 |

**`groundTexture` 是本批新增的 `MapDef` 字段**（纯表现，sim 不读，与 `envPreset` 同族）：
客户端 `groundOf()` 把它验成 `GROUND_TEXTURES` 的键，**不填或拼错回落 `stone`** ——
也就是这个字段出现之前 `NetworkScene`/`TestbedScene` 里硬写的那一行，所以老图逐帧不变。

**适配档位是被 11.2 逼出来的，不是随手划的。** 一张图尺寸固定，而梯子上每档
都有自己的标准「出生点→中央」秒数（1v1 是 5 秒、12v12 是 14 秒）。所以一张图
能盖几档 = **它的实测秒数到那几档标准秒数的距离都 ≤ 1.5 秒**（与 `maps.test.ts`
对试炼环的实现余量同一个数）。梯子小人数段间隔 1 秒、大人数段 0.5 秒 ——
于是小图只盖两档、大图能盖五档。四张合起来把 1v1–12v12 **盖满**，
8v8/9v9 两档有两张可选（`themed.test.ts` 有一条断言钉住「不留空档」）。

> ⚠️ 实测秒数 ≈ 声明秒数 + 0.29：出生点在准备区中心（z = arenaHalf − 4 = half + 2），
> 比 `half` 远 2 米。这是试炼环沿用下来的外壳形状，本批不改 —— 所以 spec 里的
> 秒数是**倒着填**的，填完由测试正着验。

**★★ 默认路径一字不变**：`ALL_MAPS` 里四张主题图排在 `ARENA_MAPS` **之后**，
于是 `setMode` 取的 `mapsForMode(mode)[0]` 仍然是试炼环那张，房间默认仍是
`arena_3v3`。主题图要玩家在选图 UI 里**主动挑**才会用上。

**三条硬约束**（`themed.test.ts` 四张图一起跑，每条对应一个会坏掉的具体方式）：

| 约束 | 落地手段 | 机检 |
|---|---|---|
| z 镜像公平 | **只写 +z 半场**，−z 半场由 `mirrorZ` 生成（id `_s` → `_n`）—— 与 §8.3 `mirrorX` 同款思路，轴换成 z。跨中线的件必须自己就 z 对称 | 逐件：`_s` 有孪生且 z 反号/x 与高度一致；无后缀的件 `min.z === −max.z` |
| 所见即所中（§8.2）| 挡路的一律是 `geometry` 真碰撞体；`decor` 只放不挡路的小件。半高掩体（≤ 胸口 1.35m）必须显式 `blocksSight:false`；体量大到「看起来能挡住人」的 decor 必须身位内有真碰撞体 | 逐件断言落地矮件的 `blocksSight`；**decor 尺寸从 glb 现量**（accessor min/max × 节点变换 × `scale`）后逐件找碰撞体；装饰模型逐个查素材盘是否存在（拼错会被 `DecorRenderer` 静默吞掉）|
| 出生走廊留空（bot 无寻路）| `\|x\| ≤ 准备区半宽` 且 `\|z\| > 中央件半径` 的带子上一件不放；中央件是例外 —— 它被**整圈**级高 ≤ 0.45m 的阶梯包着，任何方向直走都是踏上去而不是撞停 | **行为断言**：每个出生点放进 `stepMovement` 直走 40 秒，必须过中线；每处台顶给一条正面 + 一条斜向的登顶行为 |

其余机检：桥下净空 ≥ 角色 2.0m、地形件不外溢/不压准备区、地形件互不穿插
（共面相邻可以、体积重叠不行）、`pillar` 间通道 > 4×`HITBOX_RADIUS`、
大门同开、准备区开门后禁入、装饰 z 镜像且不压走廊、
**没有落在「跨不过又跳不上」死区里的落地件**、**直线陷阱率 < 6%**。

**熔岩是纯视觉**：`props/lava_*` 只摆在打不到的边角，脚下那圈是石台 ——
现在没有地表伤害系统，把岩浆铺在战场中央会造出「看着烫脚却没事」的谎。
真要能烫脚需要一套地面持续伤害区（未实现，见 docs/15）。
> ⚠️ 而且**当前熔岩件只有 ~1 米见方的点缀**（实测 `lava_pool` 0.98×0.32×0.89m、
> `lava_river_a/b` 与 `lava_terrace` 同量级，`scale` 未设）：一张叫「熔岩裂谷」的图，
> 全部熔岩加起来不到 4 平米，主题在场上基本看不见。要么把 `scale` 提到 6–10
> 成片铺，要么撤掉、把「熔岩」落在 `groundTexture=rock` + 台顶火盆 + 黑曜石尖上。
> （审计 minor，本批不改 —— 属于「贴图半边」那一堆，见 docs/17 P5。）

#### 10.6a 选图接线（2026-08-10，紧接上一批）

四张图做完之后还差一件事：**玩家进不去**。房间里没有任何路径能把 `mapId` 改成
试炼环以外的值 —— 与「护盾做完了却没有任何路径能触发」（PROGRESS 的 B4）
同一种缺陷。这一批把那条路接上：

| 环节 | 落点 | 要点 |
|---|---|---|
| 协议 | `SetRoomMap { mapId }`（客户端 → 服务器）| 走 M10 全套：`ALL_CLIENT_MESSAGE_KINDS` 登记、`Session` 的 **ROOM_ONLY** 阶段白名单、codec 校验（1–32 字符，与 `JoinRoom.roomId` 同族的长度约束）、sim 侧 `started` 守卫（A8 的纵深）|
| 规则 | `sim/match/room.ts` 的 `setMap()` | 房主专属 · 开赛前 · **地图存在** · **适配 `room.config.mode`**（判据是 `mapsForMode(mode)`，按 **id** 比对 —— m5 #24）。⚠️ 不合法**诚实拒绝**，绝不顺手换成一张能用的 —— 静默改的表现是「我明明选了熔岩裂谷，开局却在试炼环」。★ 换图**不**取消准备（换的是地形，不是「打什么、几个人」）|
| 降档回落 | 同文件的 `setMode()` | 换人数档时：当前图**仍适配就留着**（密林祭坛在 3v3–5v5 之间来回换都在），不适配才退回 `mapsForMode(mode)[0]`。留着这一半是为了「选图」在 UI 上成立；回落那一半不能省 —— 6v6 用一张只支持到 5v5 的图，是一局开不起来的房间 |
| 大厅 | 房间页「地图」行 | 选项由 `mapOptionsFor(mode)` 现算（随人数档变），房主可点、非房主只读 —— 与模式/预设同一条显示规矩。**副标题与悬停详情逐字取自 `THEMED_ARENA_SPECS` 的 `style`/`terrain`**，客户端不另写一份文案（手写的那份会在改图后变成谎话，而界面撒的谎没有任何测试会红）。大乱斗房整行隐藏（FFA 固定大图，§11 口径）|

客户端**不需要任何额外接线**跟随换图：`MatchStart.mapId` 本来就是权威来源，
`NetworkScene.loadMap()` 按它查注册表、`Environment`/`groundOf`/`DecorRenderer`
全部由那张 `MapDef` 驱动，而每局一场景（`LobbyShell` 的 destroy/rebuild）
保证不会把上一局的地图带过来。

#### 10.6b 对抗校验收口（2026-08-11）

上一批交付后做了一轮对抗校验，1 blocker + 4 major 逐条独立复现后修掉。
**每一条的共同教训是同一个：机检比的是数据，而判定藏在解算器里。**
所以本轮把「走得过去 / 上得去」的断言**全部换成跑真解算器**
（`createMovementState` + `stepMovement`），数据比对只留给纯几何性质（镜像、边界）。

| 严重度 | 缺陷 | 根因 | 修法 |
|---|---|---|---|
| blocker | 三张主题图 + `arena.ts` 3v3 试点的台顶**走路上不去**（只有会跳的人上得去）| 级高用 `platH − k·RISE` **倒着算**：`3×0.45 = 1.3500000000000000888`，于是某一级顶面比「抬升后的脚底」高 1 个 ulp，`tryStepUp` 直接 `return undefined` | 顶面高度**逐级累加**（`stairTops()`），由构造保证 `tops[k] === tops[k−1] + RISE` 逐位相等；`arena.ts` 同款修 |
| blocker | 同上，**斜 45° 走**楼梯仍然上不去（熔岩裂谷斜向登顶一路滑到地图另一头）| `stepMovement` 里「要不要试着迈一步」的判据是 `滑动距离² < 想走距离² × 0.5` —— 正 45° 时**恰好等于**一半；下一 tick 撞墙清零 `velocity.z` 后比值涨到 96%，**状态自我维持** | 判据换成「只要没走满就试一次」（`movement.ts` 一行 + 注释）。`tryStepUp` 自己有三道闸，过不去的墙照样回落滑动 |
| major | 1.2m 的「半高掩体」跨不过（>0.45）又跳不上（实测跳跃顶点 1.119m），却按 §6.4 声明 `blocksSight:false` —— **反向的所见即所中** | 落地矮件的高度落在死区 (0.45, 1.35] | 统一压到 `LOW_COVER_H = STEP_HEIGHT`（真·可跨路缘）；补一条机检：落地且挡路的件不得落在「跳跃顶点 ~ 胸口线」之间，除非旁边有一级踩得上来的下级 |
| major | 出生走廊机检只验「有没有件」，真正的漏斗是**中央楼梯比出生点排面窄**（熔岩裂谷梯宽 10 vs 出生点铺到 ±10；正对台壁的出生点直走永久停在 16.6m 外）| 中央件整体豁免了走廊规则 | 楼梯一律取**满台面宽**，且四面窄梯换成**环形阶梯**（四部窄梯之间的角落凹槽会把斜着走的角色吃掉，实测雪原 34% / 熔岩 50%）；机检换成每个出生点跑一遍解算器 |
| major | 密林祭坛 28 棵树阵是直线陷阱：42% 的采样落点永远走不到中央 | 见总账 **X28**（角色被盒子的竖棱吸住，逐轴解算 + 无寻路 = 无恢复手段）| 新增「直线陷阱率」机检（GRID=8 逐格朝中央直走，< 6%）；把祭坛前最后一件硬阻挡（鹿角神龛）挪进林深处 —— **中央前的最后一段只留可跨几何**。41.9% → **3.85%**，四张图分别 1.37/3.85/1.96/3.00%，好于既有的 `arena_3v3`（11.46%）与 `ffa_melee`（6.47%）|
| major | 装饰里摆了比「有碰撞的树柱」还大的件，全部零碰撞（先祖废墟 7.1m、鹿角神龛 4.0m、台顶火盆 1.6m > 胸口线、「贴外墙的树」实际离墙 2.5–3.5m）| §8.2 红线只写在注释里，一条机检都没有 | 大件补 `MapVolume`（神龛/图腾/废墟，**碰撞体与装饰共用同一组坐标**）、超胸口的摆件缩到 ≤1.35m、贴墙件真的推到 `arenaHalf − 0.5`；机检**从 glb 现量尺寸**，换模型/改 `scale` 会自己重新判 |

★ 直线可达率的**反直觉之处**：稀释树阵没用。实测把树径 2.6 缩到 1.8，
陷阱率反而从 6.7% 涨到 11.3% —— 洞开得越大，越多直线能走到更靠里的那圈盒子上去撞棱。
真正的杠杆是「**越靠近中央的硬阻挡越致命**」：所有指向中央的直线在半径 14m 处
相邻只差 0.4m（比角色直径还小），等于每一根竖棱都必然被某条线正好蹭到。
