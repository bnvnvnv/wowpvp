...
console.log('\n── 验收 #48：低画质下关键信息仍可见（14.4）──');
{
  ...
  //   战士假人会持续用拳击控住玩家，所以等**玩家自己**身上出现控制字形。
  //   这条路既可靠，又正好同时验证了队伍框的控制显示（15.1 左侧第四项）。
  // ★ 必须**精确**回到出生点：战士假人在正前方 2.6 米，拳击距离只有 3 米。
  ...
  //   误判成"低画质隐藏了控制"：拳击的昏迷只有几秒，切档那 0.75 秒里就可能过期。
  ...
  //   战士假人只在玩家读条时才拳击，而拳击是打断+学派锁定，**不产生昏迷**——
  //   所以"等玩家被控"这条路根本等不到（前面白试了三轮）。
  //   改成主动定身战士：Tab 选中它 → 冰霜新星（自身中心 5 米定身，瞬发无需目标）
