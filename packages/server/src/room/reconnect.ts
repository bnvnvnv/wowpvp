 ...
 *
 * ⚠️ **最后一条已被设计侧推翻** —— docs/10 已知偏差 #14（2026-08-04 拍板）：
 *    断线瞬间由人机接管、重连交还、整局内令牌有效，于是「站在原地被打」
 *    与「超时淘汰」都不再发生。
 *
 *    ★★ **本文件的代码一行没改，改的是调用方传进来的宽限期。**
 *      `RoomServer` 用 `TAKEOVER_GRACE_SECONDS`（长于任何一局）登记，
 *      于是 `takeExpired()` 永远返回空数组 —— 「不再超时」这件事因此
 *      不需要在这里加任何分支。这不是巧合：本模块只做**判定**，
 *      判定的阈值本来就该由调用方给。
 *    ★ `takeExpired()` 与 `leaveImmediately()` 都**保留**：后者是 11.5
 ...
 *
 * ⚠️ 偏差 #14 拍板后这条**不再是要维护的规则**，而是一个待改的现状。
 *   ★ 但「靠依赖的缺失表达规则」这个手法本身仍然成立、也仍然值得保留：
 *     接管发生在**调用方**（`MatchLoop`，它有 World 和 botController），
 *     本模块继续只产出名单。改成「这里直接派 AI 顶上」会同时打穿
 *     「断线不给无敌」那条 —— 拿得到 World 之后，那条就不再是写不出来了。
 */
