 ...
 *
 * ⚠️ **最后一条已被设计侧推翻，但本文件尚未改** —— docs/10 已知偏差 #14
 *    （2026-08-04 拍板）：断线瞬间由人机接管、重连交还、整局内令牌有效，
 *    于是「站在原地被打」与「超时淘汰」都不再发生。
 *    **当前代码仍是上面那套旧语义**（`takeExpired()` 产出待淘汰名单），
 *    新语义随 docs/14 §16b 的服务器侧接管一并交付。
 *    在那之前请按上面三条读这个文件 —— 注释描述的是**代码现在做什么**，
 *    不是它将来会做什么。
 *
