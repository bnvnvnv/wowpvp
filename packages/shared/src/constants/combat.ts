...
// ── 6.6 锁定投射物（W23 法术弹道 / W25 物理远程）───────────────────
/**
 * 「打出去的东西要飞到才结算」这条规则的速度表。
 *
 * ★★ **速度必须与客户端表现用弹体读到的是同一个数**，否则玩家看到的
 *   抵达时刻与伤害落账时刻会分家 —— 而那正是用户报的原始现象
 *   （「法术还没到，伤害就出来了」）的镜像版本。所以速度全部住在 shared：
 *   `client/src/vfx/SpellVfx.ts` 不再自己写一个常量，而是从**技能数据的
 *   `lockedProjectile.speed`** 读（查不到时回落 `SPEED`），
 *   两边不可能各写一个数。
 *
 * ★ W25 起速度**按投出去的是什么**分两档，不再是一个数（用户方向：
 *   「箭该有箭的样子」）。分档的边界是物件本身而不是学派：掷出去的
 *   钝器（掷锤）和粉末（致盲）走 `SPEED`，弓弩射出的箭才走 `ARROW_SPEED`。
 *   每个技能把自己那档写在各自的 `lockedProjectile.speed` 里，
 *   这里只提供**取值来源**，不代表「同一档的技能必须一样快」。
 */
export const SPELL_PROJECTILE = {
  /**
   * 法术弹体与投掷物的飞行速度，米/秒。
   * ★ 55：最远射程 35 米飞 0.64 秒，读作「嗖」的一下 —— 慢到看得见弹道，
   *   快到不至于让每一发法术都能靠走位拖成两个节拍。
   */
  SPEED: 55,
  /**
   * 弓弩箭矢的飞行速度，米/秒（W25）。
   * ★ 75：比法术快约三成，「箭比火球快」是弓箭该有的观感；
   *   落到数字上，瞄准射击最远的 35 米飞 0.47 秒而不是 0.64 秒 ——
   *   猎人是全花名册最脆的一档，他的主力输出多等的那 0.17 秒
   *   在贴脸局里是要还的。
   */
  ARROW_SPEED: 75,
  /**
   * 「这是远程」的距离判据，米。6.1 的近战档最长 3.8 米，
   * 8 米以下不给弹道 —— 与客户端 `BOLT_MIN_RANGE` 同源。
   */
  MIN_RANGE: 8,
} as const;
