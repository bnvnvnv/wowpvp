 ...
 * 「法术要飞过去才结算」这条规则的两个数字。
 *
 * ★★ **速度必须与客户端表现用弹体的 `BOLT_SPEED` 是同一个数**，
 *   否则玩家看到的抵达时刻与伤害落账时刻会分家 —— 而那正是用户报的
 *   原始现象（「法术还没到，伤害就出来了」）的镜像版本。
 *   所以它住在 shared：`client/src/vfx/SpellVfx.ts` 直接 import 它，
 *   两边不可能各写一个 55。
 * ★ 55 m/s：最远射程 35 米飞 0.64 秒，读作「嗖」的一下 ——
 *   慢到看得见弹道，快到不至于让每一发法术都能靠走位拖成两个节拍。
 */
export const SPELL_PROJECTILE = {
  /** 锁定投射物的统一飞行速度，米/秒 */
  SPEED: 55,
  /**
   * 「这是远程法术」的距离判据，米。6.1 的近战档最长 3.8 米，
