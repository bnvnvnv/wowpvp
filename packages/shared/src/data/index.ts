...
/** ★ 只含可选职业 —— 图标表/技能签名/配平矩阵都按它穷尽校验，见 SPECIAL_CLASSES */
...
export const SKILL_BY_ID: ReadonlyMap<string, SkillDef> = buildIndex(
  REGISTERED_CLASSES.flatMap((c) => c.skills).map((s) => [s.id as string, s] as [string, SkillDef]),
);
...
export const WEAPON_BY_ID: ReadonlyMap<string, WeaponDef> = buildIndex(
  REGISTERED_CLASSES.flatMap((c) => c.weapons).map((w) => [w.id as string, w] as [string, WeaponDef]),
);
