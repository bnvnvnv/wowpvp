...
export interface AuraDef {
  ...
  persistent?: boolean;
  /**
   * A18：死亡时是否随之清除。**缺省 = true**（WoW 口径：死亡掉形态与增减益 ——
   * 否则形态类 3600 秒光环跨越死亡，`tickWorld` 第 7 步照旧按它重算生命上限，
   * 波次复活 `health = maxHealth` 直接给出增益后的满血）。
   * 显式 false 只留给「死亡↔复活机器自身的零件」（复活保护）：
   * 它们的生灭由各自机制负责，不属于「身上的状态」。
   */
