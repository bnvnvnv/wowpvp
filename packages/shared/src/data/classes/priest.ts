...
const skills: SkillDef[] = [
  {
    ...
    counters:
      '读条可被专用打断并封锁神圣系 3 秒，也可被移动、控制、强制位移和失去视线终止（7.1 / 7.3）；封锁期间快速治疗和群体驱散会一起被封，是牧师最大的软肋。',
  ...
  },
  {
    ...
    counters:
      '读条可被专用打断并封锁神圣系 3 秒，也可被移动、控制、强制位移和失去视线终止（7.1 / 7.3）；受降低治疗的减益（如致死创伤）压制；受竞技场战斗抑制影响（8.5），比赛越久单次治疗量越低。',
  ...
  },
  {
    ...
    counters:
      '一次只移除 1 个效果，可被「驱散诱饵」（先叠无关魔法减益/增益）骗掉；只认魔法类，诅咒、毒素、疾病、物理减益（如致死创伤）和降治疗都驱不掉（8.4）；12 秒冷却打不过高频施加的减益；沉默或神圣系被封锁期间不可用。',
    effects: [
      // 友方目标：移除 1 个可驱散的魔法负面
  ...
  },
  {
    ...
    counters:
      '1.5 秒读条可被专用打断并封锁神圣系 3 秒，也可被移动、控制、强制位移和失去视线终止（7.1 / 7.3）；可解除部分完全免疫（圣盾术、寒冰屏障），但对方可以等读条起手后再交免疫；地面指示器不能放在墙后（6.4）；范围内最多影响 5 个单位，人多时不保证驱到关键目标；45 秒冷却。',
    effects: [
      {
        kind: 'dispel',
  ...
  // 武器方案授予的技能
  {
    ...
    counters:
      '仅魔杖+圣物方案可用，换回法杖即失效；瞬发但走暗影系，被暗影系封锁或沉默期间不可用；选这套方案的代价是治疗与护盾 -10%，输出换生存。',
    // W23：飞到才结算（6.6）
    effects: [
      {
        kind: 'lockedProjectile',
   ...
   */
  {
    ...
    cost: { resource: Resource.Mana, amount: 30 },
    counters:
      '持续伤害属于魔法减益，敌方驱散魔法一次就整段抹掉（8.4）；伤害分 6 跳给出，对爆发秒杀毫无帮助；暗影系被封锁或沉默期间不可用；不造成任何控制，阻止不了近战贴身。',
  ...
  },
  {
    ...
    cost: { resource: Resource.Mana, amount: 50 },
    counters:
      '7 秒冷却决定它只是节奏键而不是爆发；暗影系封锁与沉默都能封住（7.3）；纯伤害没有任何控制，被近战贴上时它救不了自己。',
    // W23：飞到才结算（6.6）
    effects: [
      {
        kind: 'lockedProjectile',
  ...
  },
  {
    ...
    counters:
      '只覆盖自己周围 12 米：队友散开时可能一个都奶不到，而抱团又正好吃对手的范围技能；治疗量受降低治疗减益与竞技场战斗抑制（8.5）压制；12 秒冷却，无法应对连续爆发；神圣系被封锁期间不可用。',
