...
const skills: SkillDef[] = [
  ...
  },
  {
    ...
    counters: '神圣系法术：被责难以外的专用打断封锁神圣系、或自己处于沉默期间都用不出来（7.2 / 7.3）；易伤与减速是**两枚**独立的魔法减益，一次驱散魔法只带得走一枚（先清掉的是易伤），想解掉减速要么再驱散一次，要么用自由庇佑这类专解移动限制的手段 —— 它连解带 3 秒免疫；减速 30% 是全场最弱的一档，不与其他减速叠乘（8.4），也不参与控制递减；释放瞬间失去视线或超出 25 米直接失败（7.4），柱子绕视野是最省事的应对。',
    ...
    effects: [
      {
        ...
        onHit: [
          ...
          /**
           * ★★ X13：圣骑士此前是**全场唯一一个连一条减速都没有的职业**
           *   （八职业审计：战士断腿斩 40%、盗贼毒刃 50%、猎人震慑箭 40% +
           *   断筋 50%、法师寒冷族 40% + 冰锥术 50%、死骑寒缚链 60% + 凛冬号叫
           *   40%；牧师与德鲁伊各有硬控替代，见 docs/15 的 X13 行）。
           *   一个近战职业只要目标一转身跑，除了 10 米的裁决之锤（30 秒冷却）
           *   之外没有任何粘人手段。
           *
           * ★ WoW 参照：**正义审判（Judgement of Justice）** —— 审判的经典
           *   分支之一，作用正是「让目标逃不掉」。挂在同一个技能上，量级取
           *   全场**最弱**的一档（30%），因为它是 10 秒冷却、25 米、瞬发的
           *   常驻键，不该比 12 秒冷却的寒缚链（60%）更强。
           *
           * ⚠️⚠️ **为什么不直接把 moveSpeed 加进上面那枚易伤光环**：
           *   那枚是 `casterScoped: true`，而 `aura.ts` 的
           *   `effectiveModifiersOf` 在**不传 attackerId 时会整枚跳过**
           *   casterScoped 光环 —— 移动速度那条路径
           *   （`moveSpeedMultiplierOf`）恰恰不传。写进去会得到一条编译
           *   通过、数据里看得见、跑起来**完全
           *   不生效**的减速，正是本仓库反复吃亏的那类静默失效
           *   （寒缚链的 decay、疾行步的 moveSpeedFloor 各一次）。
           *   所以拆成独立的一枚：易伤只对我生效，减速对所有人生效。
           *
           * ⚠️ **代价（X13 对抗校验记账，别再重新发现一次）**：裁决因此在
           *   同一次命中里挂**两枚 `DispelType.Magic` 减益**，而全花名册唯一
           *   的「清友方魔法减益」手段（牧师 `priest.dispel_magic`）是
           *   `count: 1`，`aura.ts` 的 `dispel()` 按入表顺序取
           *   `eligible.slice(0, limit)` —— 于是**一次驱散清掉的永远是易伤，
           *   减速原样留着**。这正是 `mage.ts` 的 `FROST_CHILL` 注释里
           *   为寒冷写下的那条禁忌（「拆成两枚 ⇒ 反制链断」），法师侧遵守了，
           *   圣骑士侧因为 casterScoped 这条硬约束**做不到**。
           *
           * ★ 三条出路都称过（docs/15 的 X13 行有完整记录）：
           *   (a) 让 `effectiveModifiersOf` 支持**逐字段** casterScoped —— 需要
           *       给 `AuraModifiers` 定义一张「哪些字段与攻击者相关」的分类表，
           *       而 11-contributing §4 的「三次即入 schema」这里才第一次；
           *       为一枚光环引进一套模糊分类，正是本仓库反复吃亏的那类抽象。
           *   (c) 给 `dispel()` 引入优先级（同类别里先清减速）—— 会波及**每一次**
           *       驱散：`control.stun` / `control.fear` 也是 magic 减益，
           *       「先清减速」等于让一次驱散去解减速而不是解昏迷，是明确的退步。
           *   ⇒ 取 (b)：**保持两枚，把口径改成如实的**（counters 已写明
           *      「一次只带得走一枚，先清掉的是易伤」），并由
           *      `chillAndStun.test.ts` 的圣骑士侧用例把这条不对称钉住 ——
           *      哪天 (a) 真的落地，那条测试会红，提醒把文案改回去。
           */
          {
            kind: 'applyAura',
            aura: {
              id: 'paladin.judgement.slow',
              name: '制裁',
              kind: 'debuff',
              duration: 4,
              dispelType: DispelType.Magic,
              clearableByTrinket: false,
              modifiers: { moveSpeed: 0.8 },
              description: '移动速度降低 20%。',
            },
          },
        ],
      },
    ],
    description: '投出神圣审判造成伤害，使目标 4 秒内额外承受 10% 来自你的伤害，并使其移动速度降低 20%。',
  },
  {
