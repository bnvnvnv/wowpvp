...
const skills: SkillDef[] = [
  ...
  },
  {
    ...
    requiresLos: true,
    counters:
      '不能穿墙、跨越巨大高差或把目标拉入非法区域，路径不合法时拉拽直接失败（6.4 / 13.5）；需要视线，柱子与门后即可躲开；属于魔法技能，沉默或暗影系被封锁期间不可用（7.2 / 7.3）；拉拽计入 8.2 击退/拉拽递减，短时间内连续使用效果衰减；完全免疫、法术免疫可以无视。',
    effects: [
  ...
  },
  {
    ...
    counters:
      '普通减速不能被「战斗意志」解除（8.3），但属于移动限制，可被自由祝福、驱散移动限制类效果或免疫新减速的状态摆脱；减速随时间衰减，拖过 4 秒就基本失效；受减速叠加规则限制，不与其他减速叠乘；魔法技能，沉默与冰霜系被封锁期间不可用。',
  ...
  },
  {
    ...
    requiresLos: true,
    counters:
      '受昏迷递减，15 秒内连续昏迷 100%→50%→25%→免疫（8.2）；可被「战斗意志」解除（8.3）；抗控型护甲缩短持续时间；需要视线，墙后无法起手；魔法技能，沉默或暗影系被封锁期间不可用；完全免疫与法术免疫直接无效。',
    // W23：暗影之手要伸过去才掐住（6.6）
    effects: [
      {
        kind: 'lockedProjectile',
  ...
  },
  {
    ...
    requiresLos: true,
    counters:
      '目标未在施法、或施法带盾牌标记（不可打断）时仍会进入冷却（7.2）；假读条可以骗掉（7.5）；打断物理射击准备条只取消本次射击，不封锁任何系；本身是冰霜魔法，被沉默或冰霜系被封锁时无法使用（7.3）；需要视线且射程只有 15 米。',
  ...
  // 武器方案授予的技能
  {
    ...
    counters:
      '仅双持符文刃方案可用；要求贴身并面向目标；虽是武器技能但结算冰霜魔法伤害，会被反魔法护罩、法术免疫和吸收吃掉，同时缴械和冰霜系封锁都能封住它（7.3）。',
   ...
   */
  {
    ...
    counters:
      '**不免疫控制** —— 与正式服不同，这里的冰封坚韧不免疫昏迷：扼喉链、裁决之锤、昏击照常把死骑按住，减伤只是让他死得慢一点；只有 5 秒（比战士盾墙短 3 秒），对手拉开等它过期即可；**120 秒冷却比竞技场单回合上限（90 秒）还长**，一回合只有一次；冰霜魔法技能，**沉默或冰霜系被封锁期间用不出来**（7.3）—— 法师一发断法锁冰霜 4 秒就能把它连同寒缚链、凛冬号叫一起封死，这是它与抗咒护罩（暗影系）分属两系的意义，也是它自己的破绽；不可驱散，但也洗不掉身上已有的减益。',
    effects: [
      {
        kind: 'applyAura',
        target: 'self',
