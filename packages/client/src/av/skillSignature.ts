 ...
 *   法系应该是有火焰燃烧的声音，其他职业也一样，声音要贴合技能」）：
 *   八张表逐条过了一遍，**16 个技能、18 个音效键**有改动
 *   （死骑 5 / 法师 4 / 猎人 2 / 盗贼 2 / 德鲁伊 1 / 战士 1 / 圣骑士 1，
 *   牧师与大乱斗派对表复核后一条没动），定下一条**选音优先级**，
 *   后人加技能请照这个顺序走：
 *     1. **学派素材优先** —— 技能是什么学派，就先在那个学派的
 *        cast_/impact_/proj_ 三件套里找。P3 那批的通病是把「唯一性」排在
 *        第一位，于是往学派外面借，借来的东西好听但不是那个技能。
 *     2. **动作素材次之** —— 学派里挑不出差异时，用这个技能**身体上在做什么**
 *        的素材（甩手 = melee_swing_light_*、撒粉落地 = foot_dirt_*、
 *        踩碎冰 = foot_snow_*、机括 = lockpick_*）。
 *     3. **借小怪音要过三关** —— ① 场上真的有小怪，玩家会误判成刷怪；
 *        ② 同一个 mob 文件常被多个职业借走，跨职业撞脸；③ 原长压速后拖尾。
 *        本轮只留下「吼的确实是他自己」的那一组（德鲁伊兽形三条、
 *        大乱斗食人魔大锤、牧师精神尖啸）。
 *     4. **唯一性靠门禁不靠选音** —— 元组撞车有
 *        `integrity.test.ts` ④ 兜着，不需要在选音阶段为它妥协语义。
 *   ⚠️ 校准的是**语义**，不是听感：全批仍然没有人耳听过。这轮的判断多了
 *     一层实测（时长 ffprobe、响度 ffmpeg volumedetect 逐条量过），
 *     但「好不好听」只有真机能回答 —— 那一半仍挂在 X23。
 */
