...
export const signatures: Record<string, SkillSignature> = {
  ...
  /**
   * 义愤（60s）—— 职业身份技，「翅膀」。
   * 为什么是这个声音/形态：用学派本身的 `cast_holy` 降到 0.8（2.30 → 2.87 秒）——
   * 圣骑士全表十六条里**没有一条**直接用过神圣咏唱（神圣键各自换了法杖 / 治疗 /
   * 金属音），把这唯一一次留给义愤，「他开的是纯粹的圣光」就成立了；
   * 降速让咏唱铺开成宣告而不是一发法术，正对 10 秒的爆发窗口。
   * 命中用提速的 spell_nova（羽翼张开推开空气），spiral 形态取地基注释
   * 里点名的「增益/蓄力」语义，规模 1.8 对应它 10 秒 +20% 的分量。
   *
   * ★★ X23 语义校准（2026-08-10）：原来用的是 `ui_gather_legendary`
   *   （传说品质采集的拾取华彩）降到 0.8，注释自己就标了「占位、凑纹理」。
   *   这轮量出来它是**全批签名里最长的一条起手**：素材 3.29 秒，
   *   ÷0.8 = **4.11 秒**。一个瞬发技能的施法音要响四秒多，
   *   下一个 GCD 都转完两轮了它还在响 —— X23 只点了 mob_* 吼叫的拖尾
   *   （最长 1.84 秒），真正拖尾的这条反而没在名单上。
   *   顺带解决出戏：竞技场里响一段「你挖到了传说材料」实在难圆。
   * ⚠️ 代价如实记：换掉之后**丢了「上扬」这个轮廓** —— cast_holy 是平铺的
   *   咏唱床，不是渐强的华彩。理想仍是一条专属展翅音（保持「上扬 + 金色 +
   *   约一秒」三特征）；在那之前，学派正确 + 不拖尾优先于轮廓漂亮。
   */
