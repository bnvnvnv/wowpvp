...
export const signatures: Record<string, SkillSignature> = {
   ...
   *   为什么是这个声音：`mob_humanoid_aggro_2` 降速到 0.78（0.81 → 1.04 秒）——
   *   人形生物的战吼降速后嗓音下沉、体型变大，而战士**就是个人**，
   *   「破胆」要的是一个人吼出来的东西；命中用降速的 cast_shadow 铺一层暗影气浪
   *   （恐惧的通用听觉符号），再叠 spell_nova 交代 8 米范围的冲击波。
   *   为什么是 Ring：以自己为圆心、半径 8 米、最多 5 人 —— 教科书式的新星，
   *   环的外沿就是"谁被吼到了"的判定可视化。
   *   色相 -0.08 吃满钳位：全表唯一一个往暗侧拉满的键，配得上 90 秒。
   *
   * ★★ X23 语义校准（2026-08-10）：原来用的是 `mob_demon_aggro_1` @0.78。
   *   三条理由，第二条是这轮才发现的：
   *   ① 战士没有恶魔血统，一个板甲人类张嘴是恶魔嗓，是全表最出戏的一处；
   *   ② **牧师的惊惧尖啸用的是同一个文件、同一个 0.78 倍速** —— 两个职业的
   *      恐惧键开口一模一样（`integrity.test.ts` 的元组唯一性查不到这种撞车，
   *      因为它比的是八元组，命中音/色相/规模不同就算过关，而玩家先听到的
   *      恰恰是那一声起手）。改这条同时把牧师那条从撞脸里摘出来；
   *   ③ 实测原素材 1.44 秒，压到 0.78 是 **1.84 秒**，是全批签名里最长的一条
   *      小怪吼叫 —— X23 担心的拖尾在这条上是真的（点名的死骑三条反而都 < 1.7 秒）。
   *      换成人形吼叫后是 1.04 秒。
   */
  'warrior.intimidating_shout': {
    castSound: 'mob_humanoid_aggro_2',
    castRate: 0.78,
